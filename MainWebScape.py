...
master_list_of_search_terms = ('unilever', 'customer support', 'customer experience', 'data center', 'help desk', 'technician',
        ...
        if use_custom_locations:
            for locale in custom_search_locations:
                ...
                for page in range(0, pages):
                    ...
                    if scrape_monster:
                        target_url = Monster.get_url(term, page, locale[0], locale[1])
                        master_job_list.append(Monster.get_job_posts(target_url, term))
                        print('Monster            pg ' + str(page + 1))
            ...
            for city in range(0, num_cities):
                ...
                for page in range(0, pages):
                    if scrape_indeed:
                        ...
                        print('Indeed            pg ' + str(page + 1))
                    if scrape_monster:
                        target_url = Monster.get_url(term, page, search_locations_usa.loc[city, 'city'],
                                                    search_locations_usa.loc[city, 'state_id'])
                        master_job_list.append(Monster.get_job_posts(target_url, term))
                        print('Monster           pg ' + str(page + 1))
else:
    ...
    try:
        ...
        if export_to_csv:
            ...
            if job_found_count > next_file_save_count:
                ...
                now = datetime.datetime.now()
                new_csv_file = open(
                    "ForkJobExport-" + str(now.day) + str(now.hour) + str(now.minute) + str(now.second) + "-" + str(csv_file_count) + ".csv", "w",
                    newline='')
                writer = csv.writer(new_csv_file)
                writer.writerow(fields)
    except:
